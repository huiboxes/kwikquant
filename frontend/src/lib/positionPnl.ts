 ...
 *
 * PositionDto.unrealizedPnl 是 decimal string(金额字段一律字符串化)，运行时可为
 * null(行情不可用，见 api-gen PositionDto 注释"行情不可用时为 null")。本函数把
 * null 视为"该仓位无法估值":
 ...
 * unrealizedPnl 的对象数组(string|number 皆可，toDecimal 统一转换)。
 */
export function sumUnrealizedPnl(
  positions: readonly { unrealizedPnl?: string | number | null }[] | undefined | null,
): Decimal | null {
