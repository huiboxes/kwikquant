...
/**
 * useAssistantChat — AI 对话 SSE 流式 hook,assistant-ui ExternalStoreRuntime adapter 层。
 *
 ...
 *
 * 复刻 spec  七项职责(从旧 useStreamChat 迁移):
 ...
 *  7. idle timeout 60s
 *
 * editor 模式 sourceCode 从 editorCodeRef 读(ref 非 props,1MB 高频 onChange 不 setState 致 re-render 雪崩,spec  m3)。
 * codeSource state(版本切换器,默认 EDITOR;draft/published 后端注入 sourceCode,不传)。
 */
...
export function useAssistantChat(
  ...
  /** 累积 chunk 到最后一条 assistant content(partial update,Thread 增量渲染,spec  第 1 项)。 */
  ...
  const onRun = useCallback(
    (text: string, llmKeyId: number | null) => {
      ...
      // placeholder assistant(content='')— streaming 进 messages,Thread 增量渲染
      ...
      const body = {
        ...
        // codeSource 分支:editor 前端传 sourceCode(编辑器实时 code);
