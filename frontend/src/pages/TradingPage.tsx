 ...
 *  - :PositionDto 无 uPnl/currentPrice(只有 realizedPnl)→ uPnl 列显示 "—"。
 ...
 *  - :平仓=反向市价单未实现(后端无平仓端点)→ ConfirmDialog destructive 占位 toast。
 *  - :重置 PAPER 后端无 reset 端点 → AlertDialog destructive 占位 toast。
 *  - :WS 推送(/topic/orders + /topic/fills)未接,用 react-query invalidate 刷新。
 *  - :OrderBook + K线 静态 mock(后端无订单簿端点;K线接真实 useKlines 待补齐)。
 *
...
function BalanceBar({
  ...
  // :PositionDto 无 uPnl,BalanceBar 第 4 格"未实现盈亏"显示 —
  const uPnlDisplay = '—'
