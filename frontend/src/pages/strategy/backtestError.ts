 ...
 *
 * 后端 Worker 跑回测引擎，失败时通过 WS BacktestEvent.error 推回原因(见 docs/ws-contract.md)。
 ...
 *  - 后端没给原因 → 兜底 error
 *
 * 注：后端把 trades empty 从 FAILED 改成 COMPLETED + 空 report 后，
...
/** 后端结构化失败上下文(FAILED 事件/任务 DTO 携带):userMessage 为分类映射的产品文案。 */
export interface BacktestFailureContext {
  category?: string | null
  userMessage?: string | null
}

/**
 * 把后端 error 文案映射成用户可读的 toast 内容。
 * 优先级：后端分类映射的 userMessage(产品文案) > 已知关键词模式 > 原样透出(真实异常原因)。
 */
export function mapBacktestError(error: string | null | undefined, ctx?: BacktestFailureContext): BacktestFailure {
  if (ctx?.userMessage) {
    // 后端按 failureCategory 映射的产品文案，直接信任；无成交类仍走 warning 由模式兜底
    return { title: '回测失败', description: ctx.userMessage, tone: 'error' }
  }
  if (!error) {
    return {
      title: '回测失败',
