...
interface SessionPanelProps {
  ...
  version: number | null
  /** 编辑器实时 code ref(父组件编辑器 onChange 写 ref.current,不 setState)。接线 RightPanel。 */
...
export function SessionPanel({ strategy, version, editorCodeRef }: SessionPanelProps) {
  ...
  // 父组件传 editorCodeRef;未接线时 fallback 空 ref(editor 模式 sourceCode=null,后端 @AssertTrue 拒 EDITOR+null,
  // 接线后正常)。useState lazy init 稳定对象(不随 render 变,避免 hook deps editorCodeRef
  ...
  return (
    <div className="m-xxs flex flex-1 flex-col overflow-hidden rounded-xl bg-surface-card">
      ...
      {/* model + 代码版本切换(Composer 上方一行;版本=策略代码来源 editor/draft/published,spec  M5) */}
