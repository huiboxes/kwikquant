 ...
 *  - EquityCurve → usePortfolioEquityCurve(⚠ mock, 后端无 portfolio/equity-curve 端点)
 *  - 策略行 pnl/version/lines → StrategyDetailDto 无:pnl 占位 "—",version 占位 "v1",lines 删
 *    (PositionPnl 无 strategyId,无法按策略聚合;待后端补策略持仓聚合端点 or TradingPage 阶段)
 *  - 4 Stat(累计收益/夏普/最大回撤/胜率)+ Hero "7 天 +12.43%" → 后端无 dashboard 聚合端点:
        ...
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div className="max-w-[600px]">
            ...
            <p className="mt-2.5 max-w-[540px] text-body-sm leading-[1.6] text-text-secondary">
              ...
              {/* 注:runningCount 真数据,"7 天 +12.43%" 占位 */}
            </p>
  ...
  return (
    <div className="grid grid-cols-[1fr_80px_100px_90px_100px] items-center gap-3 border-b border-border-soft py-3 last:border-0 max-[760px]:grid-cols-1 max-[760px]:gap-1.5">
      <div>
        ...
        {/* 注:s.timeframe→intervalValue 适配;s.version 后端无占位 "v1";s.lines 删 */}
      ...
      <div>
        ...
        {/* 注:StrategyDetailDto 无 pnl 字段,占位 "—" 待后端补策略聚合端点 */}
      ...
      <SectionTitle
        ...
        right={
          <Tabs defaultValue="30D">
            ...
            {/* 注:范围切换为视觉态,切 tab 不换数据(equityCurve mock 一份,/006 待后端补范围参数) */}
          </Tabs>
        }
      />
      <EquityCurveChart
        ...
        color="var(--accent)"
      />
      {/* 注:4 Stat 为占位(后端无 dashboard 聚合端点,),照原型数字静态展示 */}
