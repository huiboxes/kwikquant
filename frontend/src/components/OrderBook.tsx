...
/**
 * OrderBook — 订单簿深度组件(TradingPage 经 TradingOrderBook 真数据 wrapper 使用)。
 *
 * 视觉结构:
...
export function OrderBook({
  ...
  asks: OrderBookLevel[]
  bids: OrderBookLevel[]
  /** 最新价(TradingOrderBook 从 useSymbolSnapshot 取，WS tick 实时更新)。 */
  last: number
  /** 24h 涨跌%(决定最新价涨跌色)。 */
  pct: number
  loading?: boolean
  ...
  if (error) {
    return (
      <Card className="flex flex-col p-0">
        <OrderBookHeader symbol={symbol} badge={badge} />
        <div className="px-3.5 py-6">
          <ErrorState message="订单簿暂不可用" onRetry={onRetry} />
        </div>
      </Card>
    )
  }
