 ...
 *  - order book 后端无端点,MarketPage 硬编码 mock
 *  - Heatmap 多周期后端无(ticker 单点 percentage),派生 mock
 *  - subscribe/unsubscribe WS 推送管理推 marketStore 补全,当前 POST 占位 toast
 */
...
/** WS 订阅(body SubscribeRequest)。注:WS 推送管理推 marketStore 补全,当前 POST 占位。 */
 ...
 * 注:TD:后端新加 KlineSubscribeRequest record,api-gen.ts 还没更新(后端窗口 gen:api);
