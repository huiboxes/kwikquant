 ...
 *
 * 本模块当前只含 changePassword 一个端点。login/register/refresh/logout 仍在
 * hooks/useAuth 裸调 /api/v1/auth/*(后续重构统一到 api/auth.ts)。
 * SettingsPage 是 change-password 唯一消费方，本模块最小化只导出此端点，避免触碰已稳的
 * auth 链(401 单飞续期 + refresh 重放，见 docs/behavior-contract.md 鉴权流程)。
 */
