...
const ORDERS: OrderDetailDto[] = [
  // PAPER account 1 — 活动单 + 已成交
  {
    ...
    timeInForce: 'GTC',
    expireAt: '',
    // 3.1 regen 加字段(OrderDetailDto);SPOT 给零值,PERP .5 持仓表/订单表测试再加合约样例。
    leverage: 0,
    marginMode: '',
    positionEffect: '',
    reduceOnly: false,
...
const POSITIONS: PositionDto[] = [
  ...
  // side 镜像 positionSide(后端  定案 4:PERP side 由 positionSide 派生);leverage/marginMode/
...
export const tradingHandlers = [
  ...
  http.post('/api/v1/orders', async ({ request }) => {
    ...
    // 风控拒 demo:LIVE 账户(paperTrading false = id 2/4)大额订单 → 4105
