...
describe('TradingPage', () => {
  ...
  it('测试网账户明确显示测试资金，不误称真金白银', async () => {
    useUiStore.setState({ tradeMode: 'LIVE', liveConfirmedThisSession: true })
    const { user } = await renderPage()
    await screen.findByText(/真金白银/)

    await user.click(screen.getAllByRole('combobox')[0])
    await user.click(await screen.findByRole('option', { name: /OKX 实盘.*测试网/ }))

    expect(await screen.findByText('测试网')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /测试资金/ })).toBeInTheDocument()
    expect(screen.queryByText(/真金白银/)).not.toBeInTheDocument()
  })
  ...
  it('空账户引导:LIVE 模式 + 无实盘账户 → EmptyState 去添加', async () => {
    ...
    expect(screen.getByRole('button', { name: /去添加账户/ })).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /买入 .*BTC\/USDT/ })).not.toBeInTheDocument()
    expect(screen.queryByRole('combobox', { name: '下单类型' })).not.toBeInTheDocument()
  })
