 ...
 *
 * 数据源:`/topic/notifications/{userId}` WS 推送 NotificationEvent(ws-contract ,
 * type: RISK_REJECTED/ORDER_FILLED/ORDER_CANCELLED/STRATEGY_STARTED/STRATEGY_STOPPED/STRATEGY_ERROR)。
 *
 * 注:WS STOMP 订阅未建(lib/ws subscribe 未实现),store 初始 mock 5 条占位。
 * WS 接后删 mock,改 addNotification 推送(WS 订阅层调 addNotification 转换 event→Notif)。
 * 风控 RiskEvent 走 notification 通道(type=RISK_REJECTED),不单独建模(ws-contract  注)。
 */
