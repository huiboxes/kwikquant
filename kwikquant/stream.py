"""StreamClient — STOMP over WebSocket(Wave 8 §3.4 + §4.3 + §3.7 runner)。
...
class StreamClient:
    ...
    def on_kline(self, exchange: str, market_type: str, symbol: str, interval: str, handler) -> None:
        """订阅 K 线 bar(后端 MarketDataService.KLINE_TOPIC_FORMAT,§3.7 runner 主通道)。

        topic = /topic/kline/{exchange}/{marketType}/{symbol-with-dash}/{interval}。
        symbol 中的 ``/`` 替换为 ``-``(与后端 onKline destination 规则一致,见 ws-contract §1)。
