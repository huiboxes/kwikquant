...
Worker 回测:``submit_backtest(task_id, order+snapshot)`` → POST /api/v1/backtests/{taskId}/orders。
...
class TradeService:
    ...
    def submit_backtest(
        ...
        *,
        ...
        request schema 严格按 POST /api/v1/backtests/{taskId}/orders 契约,含 marketType+exchange
        (Java BacktestOrderRequest 契约必需字段)。
...
def _normalize_snapshot(snapshot: dict) -> dict:
    """OHLC → 下单接口 snapshot schema。允许调用方传 Decimal/float,统一转字符串。"""
    return {
