 ...
 *
 * <p>Covers scenarios: 1 (WebSocket enabled → push), 2 (no preferences →
 * default WebSocket), and the catch-all (channel failure must not propagate).
 */
...
class NotificationServiceTest extends AbstractIntegrationTest {
    ...
    void onRiskTriggered_whenNoPreferences_defaultsWebSocket() {
        long userId = uniqueUserId();
        // No preferences configured → WEBSOCKET defaults to enabled
    ...
    void onRiskTriggered_whenChannelFails_doesNotPropagate() {
        ...
        // catch-all: WebSocketNotificationChannel.send swallows the failure internally
