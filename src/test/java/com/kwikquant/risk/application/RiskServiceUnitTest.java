...
class RiskServiceUnitTest {
    ...
    /** PERP 请求 + 指定 availableMargin(兜底测试用)。 */
    ...
    /**
     * 兜底:PERP 请求 + 账户无 MAX_INITIAL_MARGIN policy → RiskService 用默认 80%
     * ratio 评一次(fail-closed,不 auto-approve PERP)。availableMargin 足够 → APPROVED。
     *
     * <p>per-account risk_policies 表无法全局 seed 默认 policy,故用隐式默认 ratio 兜底,
     * 等价"每账户隐式 80% policy"。真正的 per-account seed 待补齐账户生命周期阶段。
     */
    @Test
