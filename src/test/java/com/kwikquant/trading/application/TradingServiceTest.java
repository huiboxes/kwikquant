...
class TradingServiceTest {
    ...
    void submitRejectsWhenRiskCheckRejects() {
        ...
        // rejectionSummary carries only rule type names — no thresholds leak to WS push
    ...
    void submitPerpOpenLong_skipsGateAndFreezesInitialMargin() {
        ...
        // OPEN_* 不查 position(不同杠杆不同 position 由 applyFill 内 findByAccountSymbolPosition 处理,
        // submit 层 OPEN_* 不预查)
    ...
    /**
     * PERP 平仓单 risk service 故障 → bypass(isPositionReducing 改判)。
     ...
     * isPositionReducing=false → 拒单)。验证改判不误 bypass 开仓单。
     */
    @Test
