...
class BacktestExecutorTest {
    ...
    void run_buyOrderFilled_deductsCashAndAddsInventory() {
        ...
        var intent = new BacktestRequest.OrderIntent(
                activateAt, "c1", OrderSide.BUY, OrderType.MARKET,
                new BigDecimal("1"), null, null, TimeInForce.GTC, null);
        ...
        if (!result.trades().isEmpty()) {
            ...
            BigDecimal finalEquity = result.equityCurve().get(result.equityCurve().size() - 1).equity();
    ...
    void run_sellOrderWithoutInventory_rejected() {
        ...
        Instant activateAt = klines.get(0).openTime();
        var intent = new BacktestRequest.OrderIntent(
                activateAt, "c1", OrderSide.SELL, OrderType.MARKET,
                new BigDecimal("1"), null, null, TimeInForce.GTC, null);
    ...
    void run_gtdOrder_expiresOnLaterBar() {
        ...
        var intent = new BacktestRequest.OrderIntent(
                activateAt, "c1", OrderSide.BUY, OrderType.LIMIT,
                new BigDecimal("1"), new BigDecimal("100"), // very low price, won't match
                null, TimeInForce.GTD, expireAt);
    ...
    void run_iocOrder_cancelledIfNotFilledImmediately() {
        ...
        var intent = new BacktestRequest.OrderIntent(
                activateAt, "c1", OrderSide.BUY, OrderType.LIMIT,
                new BigDecimal("1"), new BigDecimal("1"), // extremely low price
                null, TimeInForce.IOC, null);
    ...
    void run_buyThenSell_fullCycle_producesFills() {
        ...
        var buyIntent = new BacktestRequest.OrderIntent(
                klines.get(0).openTime(), "buy1", OrderSide.BUY, OrderType.MARKET,
                new BigDecimal("1"), null, null, TimeInForce.GTC, null);
        var sellIntent = new BacktestRequest.OrderIntent(
                klines.get(2).openTime(), "sell1", OrderSide.SELL, OrderType.MARKET,
                new BigDecimal("1"), null, null, TimeInForce.GTC, null);
    ...
    void run_multipleBuys_weightedAvgEntryPrice() {
        ...
        var buy1 = new BacktestRequest.OrderIntent(
                klines.get(0).openTime(), "b1", OrderSide.BUY, OrderType.MARKET,
                new BigDecimal("1"), null, null, TimeInForce.GTC, null);
        var buy2 = new BacktestRequest.OrderIntent(
                klines.get(1).openTime(), "b2", OrderSide.BUY, OrderType.MARKET,
                new BigDecimal("2"), null, null, TimeInForce.GTC, null);

        BacktestRequest req = new BacktestRequest(
                1L, "strat1", Exchange.BINANCE, MarketType.SPOT, "BTC/USDT",
                Interval._1h, Instant.now().minus(java.time.Duration.ofHours(10)), Instant.now(),
                new BigDecimal("1000000"), MatchConfig.defaults(), List.of(buy1, buy2));
    ...
    void run_invalidOrderIntent_rejectedAndContinues() {
        ...
        var badIntent = new BacktestRequest.OrderIntent(
                klines.get(0).openTime(), "bad", OrderSide.BUY, OrderType.STOP_MARKET,
                new BigDecimal("1"), null, null, TimeInForce.GTC, null);
        var goodIntent = new BacktestRequest.OrderIntent(
                klines.get(0).openTime(), "good", OrderSide.BUY, OrderType.MARKET,
                new BigDecimal("0.01"), null, null, TimeInForce.GTC, null);
    ...
    void run_insufficientCashForBuy_orderRejected() {
        ...
        var intent = new BacktestRequest.OrderIntent(
                activateAt, "c1", OrderSide.BUY, OrderType.MARKET,
                new BigDecimal("100"), null, null, TimeInForce.GTC, null);

        BacktestRequest req = new BacktestRequest(
                1L, "strat1", Exchange.BINANCE, MarketType.SPOT, "BTC/USDT",
                Interval._1h, Instant.now().minus(java.time.Duration.ofHours(10)), Instant.now(),
                new BigDecimal("10000"), // small capital
                MatchConfig.defaults(), List.of(intent));
    ...
    void run_nullMatchConfig_usesDefaults() {
        ...
        BacktestRequest req = new BacktestRequest(
                1L, "strat1", Exchange.BINANCE, MarketType.SPOT, "BTC/USDT",
                Interval._1h, Instant.now().minus(java.time.Duration.ofHours(10)), Instant.now(),
                new BigDecimal("10000"), null, List.of()); // null matchConfig
    ...
    private BacktestRequest baseRequest(List<BacktestRequest.OrderIntent> intents) {
        return new BacktestRequest(
                1L, "strat1", Exchange.BINANCE, MarketType.SPOT, "BTC/USDT",
                Interval._1h, Instant.now().minus(java.time.Duration.ofHours(10)), Instant.now(),
                new BigDecimal("10000"), MatchConfig.defaults(), intents);
    }
    ...
    private List<Kline> sampleKlines(int count) {
        ...
        for (int i = 0; i < count; i++) {
            klines.add(new Kline(
                    Exchange.BINANCE, MarketType.SPOT, "BTC/USDT", Interval._1h,
