...
class OpenApiSpecTest extends AbstractIntegrationTest {
    ...
    // ===== OpenAPI PoC：通用响应注入 + 枚举字段 + envelope schema =====

    /** 验证 OpenApiCustomizer 注入通用响应生效：4 通用响应在 components + 每个 endpoint。 */
    @Test
    @SuppressWarnings("unchecked")
    ...
    /** 验证枚举策略：record 字段 @Schema description 含枚举值（PoC 确认 springdoc 3.0.3 行为）。 */
    @Test
    @SuppressWarnings("unchecked")
    ...
    // ===== 验收门：swagger-parser 0 error + 抽样 endpoint 注解 =====

    /**
     * 验收门：swagger-parser 校验 /v3/api-docs 生成有效 OpenAPI 3.0 spec，0 error。
    ...
    /**
     * 抽样 OrderController + AuthController + StrategyController 三个 endpoint 的注解完整性。
    ...
    /**
     * 前置：把 /v3/api-docs spec dump 到 target/api-spec.json，
