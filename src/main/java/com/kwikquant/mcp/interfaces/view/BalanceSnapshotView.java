 ...
 * 不直接冲击 MCP Agent 契约。金额红线：free/used/total 一律字符串输出。
 */
public record BalanceSnapshotView(Map<String, CurrencyBalanceView> currencies) {
    ...
    public record CurrencyBalanceView(String free, String used, String total) {
        public static CurrencyBalanceView from(CurrencyBalance c) {
            if (c == null) {
                return new CurrencyBalanceView(null, null, null);
            }
            return new CurrencyBalanceView(str(c.free()), str(c.used()), str(c.total()));
        }
    }
}
