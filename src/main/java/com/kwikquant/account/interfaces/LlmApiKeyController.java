...
class LlmApiKeyController {
    ...
    @Operation(
            ...
            description = "需 JWT 鉴权。完整 key 加密存储（AES-256-GCM），响应仅返回末尾 4 位明文用于识别展示。"
                    + "OPENAI_COMPATIBLE provider 必须传 baseUrl 与 model；label 重复返回 400（3001）。")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "400",
            description = "参数非法、label 重复或 OPENAI_COMPATIBLE 缺 baseUrl/model（3001 VALIDATION_FAILED）")
    ...
    record CreateLlmKeyRequest(
            ...
            @Schema(description = "自定义 base URL，OPENAI_COMPATIBLE 必填，其余可不传", example = "https://api.example.com/v1")
                    @Size(max = 500)
                    String baseUrl,
            @Schema(description = "默认模型名；OPENAI_COMPATIBLE 必填，OPENAI/ANTHROPIC 可选留空用 provider 默认", example = "gpt-4o")
                    @Size(max = 100)
                    String model) {}
}
