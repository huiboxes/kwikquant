...
/**
 * Default CcxtOrderAdapter 实现。<strong>当前 占位 — spike S1/S2 验证 CCXT Java 私有 API 可用性前，所有方法均抛
 * UnsupportedOperationException</strong>。
 *
 * <p>测试场景通过 mock CcxtOrderAdapter bean 覆盖。一旦 spike 完成，此类被替换为真实 CCXT 集成。
 *
 * <p>对应 reminder 中记录的待办：CCXT Java setSandboxMode + watchOrders 私有 WS 验证。
 */
@Component
...
public class DefaultCcxtOrderAdapter implements CcxtOrderAdapter {
    ...
    public String createOrder(ExchangeAccount account, Order order) {
        ...
        throw new ExchangeException(
                "CcxtOrderAdapter not implemented; waiting for spike S1/S2 validation. See docs/.md",
                /*retryable=*/ false);
    }
