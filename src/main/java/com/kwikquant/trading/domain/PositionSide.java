 ...
 * 字段仍为 String 便于 DB 直接存,adapter 契约层用本 enum 强类型。
 *
 * <p>本 enum 仅用于 CcxtOrderAdapter 契约(setLeverage/setMarginMode/PositionSnapshot);
 * Position 聚合根的 positionSide 字段仍为 String,在调用 adapter 时转换。
 */
public enum PositionSide {
    ...
    /**
     * 从 {@link PositionEffect} 派生持仓方向:OPEN_LONG/CLOSE_LONG → LONG,OPEN_SHORT/CLOSE_SHORT → SHORT。
     *
