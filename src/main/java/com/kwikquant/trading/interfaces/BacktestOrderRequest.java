...
/**
 * 回测下单请求(Worker→Java POST /api/v1/backtests/{taskId}/orders body)。
 *
 * <p>含 {@code marketType}+{@code exchange}(Worker 从 task config 发,避免 trading 反查 strategy 的
 ...
 * 镜像 {@link com.kwikquant.trading.domain.OrderSubmitCommand} 合约字段,<b>纯契约</b>——回测 PERP 暂未支持+
 ...
 * @param positionEffect 合约四向(OPEN_LONG/OPEN_SHORT/CLOSE_LONG/CLOSE_SHORT);SPOT null。纯契约,回测 PERP + 才生效
...
public record BacktestOrderRequest(
        ...
        @Schema(
                        description =
                                "合约持仓方向（枚举: OPEN_LONG | OPEN_SHORT | CLOSE_LONG | CLOSE_SHORT）;SPOT null。纯契约,回测 PERP + 才生效",
                        example = "OPEN_LONG")
                PositionEffect positionEffect,
        @Schema(description = "合约杠杆;SPOT null。纯契约,回测 PERP + 才生效", example = "10") Integer leverage,
        @Schema(description = "合约保证金模式（枚举: ISOLATED | CROSS）;SPOT null。纯契约,回测 PERP + 才生效", example = "ISOLATED")
