...
/**
 * 回测 single-order executor。接收 Worker 逐 bar 推来的 order+snapshot,调 {@link MatchingKernel} 撮合,
 * 维护 per-taskId 内存虚拟账本,返回 Fill。**不经 OrderRouter**(回测无真实 ExchangeAccount)。
 *
 * <p>设计取舍(避免 trading→strategy 模块违规):
 * <ul>
 ...
 *   <li>不推 per-bar on_fill WS(Worker 从 HTTP response 取 Fill;Dashboard 看最终结果 via /topic/backtests/{userId})</li>
 * </ul>
 */
@Service
public class BacktestOrderService implements BacktestLedgerLifecycle {
    ...
    public Fill submit(long taskId, BacktestOrderRequest request) {
        // 回测支持 SPOT + PERP(最小方案:撮合+保证金+PnL,不模拟强平/资金费率)。
