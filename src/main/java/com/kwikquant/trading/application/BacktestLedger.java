...
/**
 * 回测虚拟账本(per-taskId 内存,§3.1)。SPOT 现金/币库存/加权均价/已实现 PnL +
 * PERP per-position 保证金桶/持仓/PnL(档位 C-3 最小方案,不强平/资金费率留账)。
 *
...
class BacktestLedger {
    ...
    /**
     * 应用成交。SPOT 按 BUY/SELL(原逻辑);PERP 按 {@link PositionEffect} 四向(档位 C-3)。
     */
    ...
    /**
     * PERP 成交(档位 C-3 最小方案)。按 {@link PositionEffect} 四向:
     * <ul>
     ...
     * </ul>
     * 不模拟强平/资金费率(留账)。PnL = (fillPrice - avgEntry) × fillQty × sideSign(LONG +1, SHORT -1)。
     */
    ...
    /** PERP 未平仓快照(档位 C-3,供 BacktestResult 组装 unrealizedPnl;当前 Python 侧 section8Json 自算,本方法预留)。 */
    Map<String, PerpPosition> getPerpPositions() {
        return perpPositions;
    }

    /** PERP per-position 账本(档位 C-3)。包私有便测试。 */
