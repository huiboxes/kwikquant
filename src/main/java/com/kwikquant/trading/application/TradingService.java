...
public class TradingService {
    ...
    public OrderSubmitResult submit(OrderSubmitCommand cmd) {
        ...
        // PERP 才查可用保证金填 availableMargin(SPOT null,省 fetchBalance 开销);
        // MaxInitialMarginEvaluator 对 PERP fail-closed,无 availableMargin → 兜底默认 80%。
        ...
        if (decision != null && decision.getVerdict() == RiskVerdict.REJECTED) {
            // rejectionSummary carries only rule type names (no thresholds) — safe for WS push.
