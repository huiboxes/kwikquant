...
public class LiveExecutor implements Executor {
    ...
    /** per-account bills 订阅取消句柄(档位 B 实盘强平/资金费率/ADL 同步);防止重复订阅。 */
    ...
    public void submit(Order order) {
        ...
        try {
            ...
            ensureBillsSubscription(account); // 档位 B:实盘强平/资金费率/ADL 同步
    ...
    public void startupSnapshot(ExchangeAccount account) {
        try {
            ...
            ensureWsSubscription(account);
            ensureBillsSubscription(account); // 档位 B:启动恢复也起 bills 订阅
        } catch (RuntimeException e) {
