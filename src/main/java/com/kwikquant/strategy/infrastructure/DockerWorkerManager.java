 ...
 *
 * <p><b>简化（决策）</b>：{@code healthCheck} 用 {@code docker inspect}（isRunning）代理，
 * 非 HTTP {@code /health}。原因：无 Python Worker 镜像（构建），无 {@code /health} 端点可探。
 * 镜像就绪后改为 HTTP GET {@code http://{containerIp}:8080/health}（5s 超时）。
 *
...
public class DockerWorkerManager implements WorkerManager {
    ...
    public boolean healthCheck(String containerId) {
        // :HTTP GET http://<container>:8081/health,5s 超时,2xx = healthy。
