...
public class StrategyCodeService {
    ...
    /** source_code 上限 1MB。也被 {@code StrategyCodeController} 的 {@code @Size} 校验引用。 */
    ...
    /**
     * 查单个代码版本含 sourceCode 正文（list 端点不含 sourceCode，前端 Monaco reload 草稿走此端点）。
    ...
    private static void requireSourceSize(String sourceCode) {
        // 用 UTF-8 字节数（非 char count），避免含中文注释的源码绕过 1MB 限制
        if (sourceCode != null
