 ...
 *
 * <p><b>stub</b>：无 {@link BacktestRunner} 实现时，{@code updateError}("回测执行待 Python Worker
 * 实现") → 推送失败事件。由 Python Worker 适配器实现 {@link BacktestRunner}，注入后自动走真实执行路径。
 */
@Component
public class BacktestExecutionGateway {
    ...
    static final String STUB_MESSAGE = "回测执行待 Python Worker 实现";
