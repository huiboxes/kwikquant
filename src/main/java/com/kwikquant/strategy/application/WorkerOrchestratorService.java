 ...
 *
 * <p>只建 Java 侧编排逻辑；Python Worker 容器本身 实现。
 */
@Service
public class WorkerOrchestratorService {
    ...
    private WorkerConfig buildConfig(StrategyDefinition strategy, StrategyCode code) {
        // :token 由 WTS 签发,绑 strategyId+taskType=RUNNER+userId+exchange,
        // WorkerTokenFilter 从 entry 直接注入 request attr 供 OrderController 推导 account。
