...
/**
 * 策略代码版本管理 REST 端点。sourceCode ≤1MB（服务层二次校验）。
 */
@RestController
...
class StrategyCodeController {
    ...
    @Operation(
            summary = "查代码版本详情",
            description = "需 JWT 鉴权。返回含 sourceCode 正文（list 端点不含 sourceCode，前端 Monaco reload 草稿走此端点）。"
     ...
     * GET /{codeId} 返此 DTO（前端 Monaco reload 草稿用）。
     */
