 ...
 *
 * <p><b>查找哈希 vs salt 的设计决策</b>：直觉方案是
 * {@code token_hash = HMAC-SHA-256(raw, pepper+salt)}（per-token 随机 salt），但 verify
 * 「先哈希再查」走 {@code uk_mcp_token_hash} 索引。per-token 随机 salt 与索引查找数学上不兼容
 * （无法在不知道 salt 的情况下预计算 salted hash 用于索引命中）。为兼顾「索引命中热路径」
 * 与「pepper defense-in-depth」，{@code token_hash} 使用 <b>pepper-only HMAC</b>（service 层
 * 调 {@link #hash(String, String)} 传空 salt，{@code pepper + ""} = {@code pepper}），salt 列按
 * schema 保留并由 {@link #generateSalt()} 填充（per-token 随机），留作未来 salted-confirmation 迁移，
 * 当前不参与查找哈希。DB-only 泄漏下连哈希计算都做不了（无 pepper），满足 defense-in-depth 意图。
 *
 * <ul>
