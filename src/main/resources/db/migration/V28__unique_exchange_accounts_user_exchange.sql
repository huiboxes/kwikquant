...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Duplicate (user_id, exchange) rows in exchange_accounts; de-duplicate before adding UNIQUE constraint';
    END IF;
END $$;
...
COMMENT ON CONSTRAINT uk_exchange_accounts_user_exchange ON exchange_accounts IS
    '每用户每交易所最多一账户, 保证 findByUserAndExchange 无歧义推导（同交易所单账户产品规则）';
