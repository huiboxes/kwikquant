...
period 回看周期,threshold 入场涨幅阈值(比例),amount 每笔下单量。
"""
from decimal import Decimal

PERIOD = int(PARAMS.get("period", 24))        # 回看周期(4h × 24 = 4 天)
THRESHOLD = float(PARAMS.get("threshold", 0.02))  # 涨幅超 2% 入场(比例,非金额)
...
def on_bar(bar, ctx):
    ...
    if roc > THRESHOLD and pos.qty <= 0 and tradable:
    ...
    elif roc < 0 and pos.qty > 0 and tradable:
