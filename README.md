...
前端仓在 `frontend/`（脚手架搭建中，见 [`frontend/.md`](frontend/.md)）。
...
**脚手架未搭完前**（当前状态）：`frontend/` 目录里只有 `DESIGN.md`（视觉契约）和 `.md`（搭建清单）。搭建进行时切到 `frontend-scaffold` 分支。
...
| [`frontend/.md`](frontend/.md) | 前端脚手架搭建清单 |
