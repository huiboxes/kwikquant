...
| `backtests submit <strategyId>` | `--start/--end`(必填 ISO-8601)、`--symbol`(单标的)或 `--symbols <s1,s2,...>`(组合 2-20,互斥)、`-e/--exchange`、`--interval`、`--params <json>`、`--allow-funding-proxy` | 提交回测(免确认,不产生成交;symbol/exchange/interval 省略回退策略绑定;组合支持 SPOT 与 PERP) |
