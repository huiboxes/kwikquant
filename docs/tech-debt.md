...
> **⚠️ 编号撞号声明（2026-07-19 整理）**
>
> 本清单存在**两套编号空间**，号段重叠、同号不同义，按所在分区区分：
>
> - **后端债 TD-xxx** —— 2026-07-19 重新连续编号。待处理区 TD-001~009，已处理区 TD-001~031。每条标题尾部标注 `(原 TD-xxx)` 便于 git 历史追溯。
> - **前端债 TD-xxx** —— 保留原号（TD-003~054），与 commit `5fac1aa` 等 git 历史引用对齐，未重排。
>
> 原 `tech-debt-frontend-gaps.md`（`FE-TD-xxx`）已于 2026-07-19 合入本文件：FE-TD-038（strategy↔account 单账户不变量）并入后端已处理 TD-031，FE-TD-049（OAuth/SIWS）并入前端待处理 TD-054；FE-TD-021/046/048 假阳性订正（后端端点原本就有）同步到对应前端条目；FE-TD-028/039 的 V2 修复状态同步到 TD-028/TD-039。该文件已删除。
...
- **RiskGate policy 只有三条** —— 后续有时间时优化，多新增几条。
- **撮合双模式可选** —— 目前为保证三态一致在 Java 撮合，后续有参数寻优需求时加双模式可选功能。
...
#### TD-001 — MatchingKernel.walkDepth 流动性不足回退价格对买单乐观（原 TD-008）
- **模块**: trading
...
- **建议**: 流动性不足时按对手盘挂单加权或拒绝成交。
- **优先级**: 低

#### TD-002 — computeProportionalFrozen 多次等量 partial fill 微量尾差（原 TD-028）
- **模块**: trading
...
#### TD-003 — Portfolio Summary 对纯 Paper Trading 用户返回空（原 TD-034）
- **模块**: report
...
#### TD-004 — TradingService.submit() / processExecutionReport() 方法过长（原 TD-035）
- **模块**: trading
...
#### TD-005 — 多处 public 方法参数 >3（原 TD-036）
- **模块**: trading, account
...
#### TD-006 — MarketDataService 应用层直接依赖 8 个 infrastructure 类（原 TD-037）
- **模块**: market
...
- **建议**: 抽取 MarketDataPort，应用层只依赖接口，Worker 创建委托工厂。
- **优先级**: 中（架构合规，当前功能不受影响）
- **发现方式**: excellence-engineer 巡检

#### TD-007 — BalanceService.createAuthenticatedExchange CCXT SDK 耦合（原 TD-038）
- **模块**: account
...
- **建议**: 收口到 `CcxtExchangeRegistry` 统一提供已认证实例。
- **优先级**: 低（扩展性问题，当前三交易所够用）
- **发现方式**: excellence-engineer 巡检

#### TD-008 — DockerWorkerManager.createAndStart 子进程 stdout 管道死锁风险（原 TD-039）
- **模块**: strategy
- **位置**: `DockerWorkerManager.java` `createAndStart()` 的 `docker run` 子进程调用
- **问题**: `redirectErrorStream(true)` 后 `waitFor()` 在 `readAllBytes()` 之前调用，与本仓库已知并修复过的 `RealSubprocessExecutor` 管道死锁属同一模式（子进程 stdout 超过 OS 管道缓冲区 ~64KB 时子进程写阻塞 → `waitFor()` 永远等不到退出）。正常 `docker run -d` 输出很小，但本地镜像需要 pull（打印大量 layer 进度）或启动失败输出较长错误堆栈时会触发，且 `waitFor()` 无超时保护，会永久挂起调用线程。
- **修复方向**: 仿照 `RealSubprocessExecutor` 的异步 drain 模式，在 `waitFor()` 之前用独立线程/虚拟线程异步读 stdout，并给 `waitFor()` 加超时。
- **优先级**: 中（当前本地开发镜像已预先 build 好，触发概率低；CI/生产环境首次拉镜像时风险更高）
- **发现方式**: excellence-engineer 巡检（2026-07-15 全后端 9 模块巡检）

#### TD-009 — WorkerOrchestratorService.restartStrategy 与并发 stopWorker 竟态（原 TD-040）
- **模块**: strategy
- **位置**: `WorkerOrchestratorService.java` `restartStrategy()`（健康检查失败次数低于 `MAX_FAILURES` 阈值时触发）
- **问题**: `restartStrategy()` 直接 `registry.put(strategyId, failed.withContainer(newContainerId, ...))`，未像同类的 `handleUnhealthy()` 达到 `MAX_FAILURES` 分支那样用 `registry.compute()` + containerId 匹配校验防止覆盖并发变更（该分支正是已处理 TD-010 修复过的模式）。若健康检查重启期间用户调用 `stop`（`registry.remove` + 停容器 + revoke token），`restartStrategy()` 之后仍会把新建容器塞回 registry，造成 DB 状态已是 STOPPED 但内存 registry 出现孤儿容器 + 新签发 token 的不一致。
...
- **建议**: 后端补 `/api/v1/portfolio/equity-curve` 真端点，或澄清曲线来源。前端只需改 handler + URL，page 不变。
- **优先级**: 中
...
- **问题**: 前端 AlertDialog destructive 占位 toast"待后端提供 reset 端点"，不发明组合逻辑（平仓+删单+改余额）。
- **后端现状**（2026-07-19 订正，原 FE-TD-004/045 假阳性）: 后端 `POST /api/v1/accounts/{id}/paper/reset`（trading 模块 `PaperAccountController`）已实现——清订单+清仓+回 10 万 USDT。此 TD 实为前端接线债，非后端缺端点。
- **影响**: 重置 PAPER 按钮点击只 toast 不生效。
- **建议**: 前端 reset 按钮接 `POST /accounts/{id}/paper/reset` 真调，ConfirmDialog 二次确认。page 不变。
- **优先级**: 中
...
- **建议**: 后端 `StrategyDetailDto` 补 `pnl`/`version`/`lines` 字段（或返策略持仓聚合），前端接字段。
...
- **建议**: marketStore 补 subscribe/unsubscribe + WS 接收 ticker/kline 推送 → 更新 tickerTick + ticker/kline 缓存。
...
- **问题**: 原型"导入"按钮只 toast 无 handler。前端按钮 toast.info 占位，未实现文件上传/JSON 粘贴 UI。importReport api 已建（备用）。
- **后端现状**（2026-07-19 订正，原 FE-TD-021 假阳性）: 后端 `POST /api/v1/reports/import`（`BacktestSubmitRequest`: name/params/symbol/timeframe/period/trades/equityCurve）已存在。此 TD 实为前端 UI 债，非后端缺端点。
...
#### TD-028 — SettingsPage telegram/webhook 通知渠道（V1 文档对齐已做，V2 渠道实现待办）
...
- **问题**: 原型 4 渠道 ws/email/telegram/webhook。前端 UI 保留 4 渠道照原型，PUT 只传 WEBSOCKET/EMAIL（明确支持的）；telegram/webhook 保持 UI 本地态不 PUT（刷新后回默认）。
- **V1 修复**（原 FE-TD-028）: `NotificationPreferenceRequest.PreferenceItem.channelType` 的 `@Schema` 已改为如实反映「V1 仅 WEBSOCKET，EMAIL/TELEGRAM/WEBHOOK 暂未实现，传不支持值返 400」。文档对齐完成。
- **V2 待办**: EMAIL/TELEGRAM/WEBHOOK 渠道真正实现（需后端 notification 模块补渠道发送能力）。
- **影响**: telegram/webhook 勾选不持久（刷新丢失），用户可能困惑。
- **建议**: V2 后端实现多渠道发送后，前端 PUT 传全 4 渠道；或前端删 telegram/webhook 列（只留 WEBSOCKET/EMAIL）。
- **优先级**: 中（多渠道通知是产品差异化，但当前 WEBSOCKET 已覆盖核心）
...
- **建议**: 契约澄清——查后端 `Order` 校验逻辑（price 字段对 7 orderType 的要求），结论补 `docs/behavior-contract.md` §2，前端再读。当前 MARKET_LIKE 推断覆盖 7 类型，功能可用。
...
#### TD-054 — 第三方登录（Google/GitHub OAuth + Solana SIWS）未实现（原 FE-TD-049）
- **模块**: 前端 LoginPage/RegisterPage / 后端 auth
- **位置**: 前端 `LoginPage`/`RegisterPage` 三社交按钮（Google/GitHub/Solana）已注释为占位，上线前勿开
- **问题**: 原型登录页有 Google/GitHub/Solana 三个社交登录按钮，前端已注释占位、未实现。后端无 OAuth 端点。
- **建议**:
  - **Google/GitHub**: 标准 OAuth2 Authorization Code + PKCE。后端补 `POST /api/v1/auth/oauth/{provider}/start` + `/auth/callback`。需新表 `user_identity(user_id, provider, provider_sub, provider_email)` + Flyway 迁移 + MyBatis mapper。估 ~2-3 天。
  - **Solana**（非 OAuth）: 走 SIWS（Sign-In With Solana）钱包签名验签。前端 Phantom 适配器发起签名，后端验签恢复 pubkey。crypto 域单独一套，估 ~2-3 天。
- **产品决策待定**: OAuth 用户是否绕过现有邀请码注册流；同邮箱已用密码注册时合并还是拒。
- **优先级**: 中（差异化登录，但当前密码+邀请码可用）

---

## 已处理

### 后端债（TD-001 ~ TD-031，2026-07-19 重排）

#### TD-001 — refresh_tokens 清理未调度（原 TD-001）
- **修复**: 新增 `RefreshTokenCleanupScheduler`（`@Scheduled(fixedDelay=1h, initialDelay=5m)`），调用 `deleteExpiredAndRevoked()`。

#### TD-002 — Kline upsert 无法区分实时更新与历史修正（原 TD-002）
- **修复**: V26 migration 加 `updated_at TIMESTAMPTZ DEFAULT now()` 列；upsert SQL 加 `updated_at = now()`。

#### TD-003 — freezeBalance/unfreezeBalance @Transactional(REQUIRES_NEW) 因同类自调用不生效（原 TD-003）
...
#### TD-004 — ExchangeAccountService.create 参数过多（原 TD-004）
...
- **遗留**: CcxtOrderAdapter 增加 subscribeOrderUpdates 接口；回调中对 cancel 确认事件调 confirmCancelled。
- **优先级**: 高（blocked by CCXT WS order status 订阅实现）

#### TD-016 — Paper GTC stop/take-profit/trailing 订单永不触发致冻结额永久泄漏（原 TD-017）
...
#### TD-017 — PaperExecutor.submit PENDING_NEW→SUBMITTED 恢复失败致死路（原 TD-018）
- **修复**: PENDING_NEW→SUBMITTED 改为完整 CAS 重试循环（MAX_CAS_RETRIES=3），每次重读最新状态后重试。消除单次恢复失败导致订单卡死在 PENDING_NEW 的风险。

#### TD-018 — BUY partial fill frozenQuoteAmount 按比例解冻（原 TD-019）
...
#### TD-021 — ResourceStateConflictException 孤儿 NEW 订单（原 TD-022）
- **修复**: TradingService.submit 捕获 freezeBalance 抛出的 ResourceStateConflictException（CAS 耗尽），reject 订单避免孤儿 NEW。

#### TD-022 — GTD scheduler 漏扫 PENDING_CANCEL + 状态机缺 EXPIRED（原 TD-023）
...
#### TD-023 — PaperExecutor cancel CAS 耗尽自愈失效（原 TD-024）
- **修复**: onTicker 对 PENDING_CANCEL 订单做终态检查+移除，使 GTD expire 推到终态后能从 activeOrders 清理。

#### TD-024 — cancel unfreeze guard 条件过宽致正常撤单不解冻（原 TD-025）
...
#### TD-028 — WorkerTokenFilter 劫持 JWT 用户的 /api/v1/orders 请求（原 TD-030）
...
#### TD-029 — Logout 不撤销 Access Token，JWT 在有效期内仍可使用（原 TD-033）
...
#### TD-031 — strategy↔account 单账户不变量（原 FE-TD-038，Option B）
- **模块**: account / trading
- **原问题**: `findByUserAndExchange(userId, exchange)` 多账户场景歧义；`exchange_accounts` 无 `(user_id, exchange)` 唯一约束。
...
> 与 commit `5fac1aa`（标 9 个前端 TD 已解决）+ `2bcf256`（task#27 删会话/轮换占位）对应。
...
- **原问题**: 后端无"列表所有 ticker"端点（只有单 symbol GET `/market/ticker/{exchange}/{marketType}/{symbol}`）。前端 hardcode MARKET_SYMBOLS（6 个主流 USDT 对）循环 useTickers（useQueries 批量 GET）。
- **修复**: 后端补 top8 tickers 端点，前端接真实数据。

#### TD-009 — MarketPage 订单簿后端无端点 → 已解决（4813606 TD-009 orderbook）
- **模块**: 前端 MarketPage / 后端 market
- **位置**: `frontend/src/pages/MarketPage.tsx` OrderBook 组件
- **原问题**: 后端无 order book / depth 端点。OrderBook 硬编码 mock（基于 sel.last 派生 asks/bids 6 行，稳定无随机）。
- **修复**: 后端补 orderbook 端点，前端接真实数据。

#### TD-010 — MarketPage Heatmap 多周期后端无 → 已解决（992fa07 TD-010 删 Heatmap）
- **模块**: 前端 MarketPage / 后端 market
- **位置**: `frontend/src/pages/MarketPage.tsx` HeatmapSection
- **原问题**: 后端 ticker 只单点 percentage，无多周期涨跌。Heatmap 多周期用 percentage 派生 mock。
- **修复**: 删除 Heatmap 区段（照原型裁剪）。
...
- **原问题**: `POST /reports/compare` 返 `ComparisonResultDto.reports`（flat，无 equityCurve 字段）。前端无各报告 equityCurve 数据，用静态占位。
- **修复**: 对比叠加 EquityCurve 接真实数据。
...
- **原问题**: `TradeRecordDto` 无 `pnl`/`equity`。前端占位 "—"。
- **修复**: 后端 `TradeRecordDto` 补 `realizedPnl`，前端接字段。
...
- **原问题**: `BacktestReportDto`（列表）无 equityCurve 字段。前端用 SPARK_STATIC 静态数据占位。
- **修复**: 列表 Sparkline 接真实趋势数据。
...
- **原问题**: 后端无会话列表端点也无吊销端点。前端会话卡静态硬编码，"吊销"按钮走 ConfirmDialog 占位 toast.warning。
- **修复**: 删会话卡（task#27 决策非核心，占位移除）。
...
- **原问题**: 后端无 rotate 端点。"轮换"语义 = 删旧 key + 建新 key 两步。前端走 ConfirmDialog 占位 toast.info。
- **修复**: 删轮换按钮（task#27 决策，手动删+建可达成）。
...
- **原问题**: 契约 `POST /strategies/{id}/start` 描述"READY→RUNNING 转移，需有发布代码"。前端也用于 PAUSED→RUNNING（resume 语义）。后端无独立 resume 端点。MSW mock 已对齐（接受 READY|PAUSED→RUNNING）。
- **修复**: behavior-contract §10 澄清 start 双接 READY+PAUSED（PAUSED→RUNNING 即 resume，复用同一端点，无独立 resume）。

#### TD-039 — 订单状态枚举不一致 → 已解决（原 FE-TD-039 V2 强类型化）
- **模块**: trading
- **位置**: `OrderDetailDto.status` / `OrderEvent.previousStatus,newStatus` / `statusChanged` 工厂 / 5 调用点 + 3 测试
- **原问题**: DTO/ws 字段为 `String`，`@Schema` 写 6 态用 `PARTIAL`，与运行时 9 态 `OrderStatus.name()` 不符；前端需 `normalizeOrderStatus` 映射层。
- **修复**: `String → OrderStatus` 强类型化，`@Schema` 9 态全枚举；`OrderController.toDto`/`ExecutionService`/`PaperExecutor`/`GtdExpirationScheduler` 去除 `.name()`；`OrderWebSocketBroadcasterTest` 入参改枚举；`OpenApiSpecTest` 重写为断言 OrderStatus 枚举 9 态。前端 `normalizeOrderStatus` 映射层可移除。
- **验证**: 编译干净；非容器测试 1002/0 failures（`OrderControllerTest` 16/16、`OrderWebSocketBroadcasterTest` 6/6）。**残留**: `OpenApiSpecTest` 9 态断言形态 + 169 Testcontainers 测试需在 Docker 可用环境跑 `./mvnw clean verify` 闭环。
...
#### TD-005 — PortfolioPage balance 降级返成功子集（非债，产品决策）
- **模块**: 前端 PortfolioPage / 后端 portfolio
- **位置**: `frontend/src/test/handlers/portfolio.ts` summary handler + 后端 `PortfolioService.getSummary`
- **说明**: portfolio/summary 端点「部分账户失败降级返成功子集」是**产品合理性决策**——不让单个账户 balance 请求挂掉（502/超时）导致整个 portfolio 面板不可用，失败账户降级返成功子集，保证主体可见。后端 `PortfolioService.getSummary` 逐账户 try/catch 降级逻辑已落地，即设计意图。当初原 TD-005 把它当债记，实为产品决策，非债。
- **可选改进**（非债，不阻塞）: 前端 MSW handler 简化为全成功，未 mock 部分账户失败降级场景。若要补测试覆盖，MSW 加一个部分账户 502 的 handler 即可，page 不变。

#### TD-020 — BacktestPage 提交 modal 撮合模式字段后端无（非债，FAST only 是回测本质）
- **模块**: 前端 BacktestPage / 后端 backtest
- **位置**: `frontend/src/pages/BacktestPage.tsx` SubmitModal 撮合模式 Select（FAST only disabled）
- **说明**: `SubmitBacktestRequest` 无 `matchMode`。后端回测永远用 FAST（最新价+滑点，behavior-contract §3 提过"回测引擎只有 K 线数据"）。前端 Select FAST only disabled（展示性，不可改）。FAST only 是设计选择，非债。V2 双模式可选属另一条后端债（见待处理杂项"撮合双模式可选"）。

#### TD-029 — SettingsPage provider 枚举映射中文（非债，前端展示层职责）
- **模块**: 前端 SettingsPage / 后端 ai
- **位置**: `frontend/src/api/ai.ts` providerLabel + `frontend/src/pages/SettingsPage.tsx` LlmKeyCard Chip
- **说明**: `LlmApiKeyView.provider` 枚举 `OPENAI|ANTHROPIC|OPENAI_COMPATIBLE`（英文大写）。原型显示中文。前端 `providerLabel(provider)` 映射函数转中文。纯展示映射，功能正常，非债。

#### TD-031 — SettingsPage MCP token 列表明文不可再次查看（非债，契约安全设计）
- **模块**: 前端 SettingsPage / 后端 mcp
- **位置**: `frontend/src/pages/SettingsPage.tsx` McpTokenCard + AddMcp→McpReveal modal
- **说明**: 契约 `McpTokenView`（列表）不含明文 token。明文 token 只在 `McpTokenIssueResult`（POST /mcp/tokens 响应）中返回一次，DB 只存哈希。原型 show/hide toggle 是 mock 行为，真实场景不可行。前端永久 masked + 提示"明文仅签发时显示一次"，是契约安全设计，非债。
...
> 移植自原 `tech-debt-frontend-gaps.md` 的 T2/T3/T4 待办分组，按"后端动工类型"重新组织，便于排期。每项标注当前状态：✅ 已解决 / 🔁 假阳性（后端原本就有）/ ⏳ 待办。纯前端 UI/测试债（不涉及后端动工）不在此列。

### T2 — DTO 字段补全（后端补字段，前端接）
...
| TD-013 | ⏳ | `/market/watchlist` CRUD（新表）|
| TD-015 | ⏳ | `GET /api/v1/backtasks?status=RUNNING` 跨策略 |
| TD-028 | ⏳ V2 | EMAIL/TELEGRAM/WEBHOOK 通知渠道实现（V1 文档对齐已做）|
| TD-054 | ⏳ | OAuth Google/GitHub + Solana SIWS（需新表 `user_identity` + Flyway 迁移）|
