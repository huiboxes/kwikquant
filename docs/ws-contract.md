...
> 资金费率结算事件(档位 B,OKX PERP 8h 资金费率结算落账)。来源 `shared/types/FundingSettlementEvent.java` record,
...
> `/topic/funding/{userId}`。仅实盘有资金费率(PAPER 不模拟,Stage2 拍板)。
...
> `type=5`(强平)/`type=9`(ADL)走 `LiquidationService.processLiquidationReport` → LiquidationEvent(§3.9)。
...
> **金额字段序列化为 number**:同 §3.9,Jackson BigDecimal→JSON number。前端 `ws.ts` 的
