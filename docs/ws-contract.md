# KwikQuant WebSocket 主题契约

> 版本:0.0.1。**本文档为前端(Dashboard)与外部 SDK 消费者共同引用的 WS 主题合约**。
...
> **PERP 合约字段缺口**:`PositionEvent` record 当前
...
> 本 schema 已按实际代码(`NotificationEventType.java` + `NotificationService.dispatch`)对齐。`title` 是 `dispatch(userId, title, payload)` 的独立参数,**不入 payload**。
...
> 本节为决策记录而非待办,记录决策代价供后续重估。

### 7.1 不引入 AsyncAPI 自动生成
...
**决策**:见第 8 节客户端连接管理。STOMP heart-beat 默认 10s ping;客户端断线指数退避重连 + 重订阅。
...
- 心跳失败(连续 2 个周期无 pong)→ 客户端判定连接断开 → 触发 8.2 节重连。
...
const client = new Client({
  ...
  // 鉴权:HTTP 握手阶段浏览器自动附带 refresh_token cookie(path=/),非 CONNECT Bearer
...
- 与 8.2 节的区别:鉴权失效不重连(重连也只会再 401),网络断开才重连。
