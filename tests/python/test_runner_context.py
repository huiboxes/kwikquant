"""RunnerContext 单元测试(§3.7 实盘/模拟盘 ctx)。"""
...
def test_place_order_calls_submit_without_exchange_account_id():
    """worker 模式不传 exchange_account_id(OrderController 据 token 推导 account,§3.7 R4)+ marketType 透传。"""
    client = MagicMock()
...
def test_place_order_perp_passes_leverage_margin_mode_position_effect():
    ...
    submit kwargs 完整透传(§3.7 R5);后端 OrderController.perp 分流 → LiveExecutor 走 swap。
    """
    client = MagicMock()
    client.trade.submit.return_value = {
        "orderId": 11,
